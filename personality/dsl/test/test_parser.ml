...
let test_trait_modifiers () =
  ...
  traits {
    focus: 0.7 with decay(0.1/day), when("work");
  }
}|} in
  match Ast.parse_personality_from_string dsl with
  | Ok personality ->
      ...
      Alcotest.(check int) "modifier count" 2 (List.length trait.Types.modifiers)
  | Error errors ->
      let error_messages = String.concat ~sep:"; " (List.map errors ~f:(fun e -> e.message)) in
      Alcotest.fail ("Parse failed: " ^ error_messages)
...
let test_full_personality () =
  ...
  evolution {
    if learns("new skill") then trait("curiosity") += 0.1;
    if interactions(10) then unlock_domain("philosophy");
  }
}|} in
  match Ast.parse_personality_from_string dsl with
  | Ok personality ->
